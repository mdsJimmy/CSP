<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage/MainPage.master" AutoEventWireup="true"
    CodeFile="MipInfoQ.aspx.cs" Inherits="yuantalife_web_YL0010Q" %>
...
<asp:Content ID="Content6" ContentPlaceHolderID="InlineScriptsPlaceHolder" runat="Server">
    ...
    function DoAdd() {
        ...
        var url = "MipInfoA.aspx?a=a&queryCondi=<%=MDS.Utility.NUtility.UrlEncode( strQueryCondi)%>";
        //alert(url);
    ...
    function DoSearch() {
        ...
        parent.location.replace("MipInfoQ.aspx?PageNo=<%=MDS.Utility.NUtility.UrlEncode(""+NewsListView.PageNo)%>&_selAppType=" + $('#<%=MDS.Utility.NUtility.UrlEncode(""+_selAppType.ClientID)%>').val() + "&_selNewsKind=" + $('#<%=MDS.Utility.NUtility.UrlEncode(""+_selNewsKind.ClientID)%>').val()
            + "&_txtStartDate=" + $('#_txtStartDate').val()+"&_txtEndDate="+ $('#_txtEndDate').val() );
    }

    /*預設按鈕按下;*/
    function DoDefault() {
        parent.location.replace("MipInfoQ.aspx");
    }
    ...
    function DoEdt(chkPK) {
        ...
        **********************************************************************/
        ...
        if (tmpArr[0] == "MIP_TOLO_CNT_02" |
            ...
            var url = "MipInfoM.aspx?a=a&queryCondi=<%=MDS.Utility.NUtility.UrlEncode(strQueryCondi)%>&buildId=" + tmpArr[0];
            parent.location.replace(url);
        }
