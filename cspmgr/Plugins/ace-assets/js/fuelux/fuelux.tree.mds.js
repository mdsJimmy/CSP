...
(function (factory) {
    ...
    var Tree = function (element, options) {
        ...
        this.$element.on('click.fu.tree', '.icon-folder', $.proxy(function (ev) { this.openFolder($(ev.currentTarget).parent()); return false; }, this)); //MDS
        //ACE
    ...
    Tree.prototype = {
            ...
            this.options.dataSource(treeData ? treeData : {}, function (items) {
                ...
                $.each(items.data, function (index, value) {
                    ...
                    if (value.type === 'folder') {
                        ...
                        //MDS
                        header.attr('data-key', value.sn); //目的:讓jQuery能透過DOM的data屬性得知該節點
                        ...
                        //MDS
                        $entity.attr('data-key', value.sn); //目的:讓jQuery能透過DOM的data屬性得知該節點
                    }
        ...
        selectFolder: function (clickedElement) {
            ...
            */
            //MDS
            if ($clickedBranch.find('.tree-branch-header').first().hasClass('tree-selected')) {
                //eventType = 'deselected';
                //$clickedBranch.find('.tree-branch-header').removeClass('tree-selected');
            } else {
                $all.removeClass('tree-selected');
                $clickedBranch.find('.tree-branch-header').first().addClass('tree-selected');
            }
