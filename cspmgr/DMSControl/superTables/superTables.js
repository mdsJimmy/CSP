...
var superTable = function(tableId, options) {
    ...
    var alpha, beta, touched, clean, cleanRow, i, j, k, m, n, p, alphaWidth;
    ...
    for (i = 0, j = alpha[cleanRow].cells.length; i < j; i++) {
        if (i === this.colWidths.length || this.colWidths[i] === -1) {
            ...
            alphaWidth = alphaWidth + this.colWidths[i];
        }
    }
    if (alphaWidth > this.sParentWidth)
        this.colWidths[i-1] = this.colWidths[i-1] - (alphaWidth - this.sParentWidth);
    ...
    if (this.cssSkin !== "") {
        ...
        this.sDataTable.className = "sData-Main";
        this.sHeaderTable.className = "sHeaderInner-Headers";
        if (this.fixedCols > 0) {
            this.sFDataTable.className = "sFData-Fixed";
            this.sFHeaderTable.className = "sFHeader-FixedHeaders";
        }
    }
    ...
    sDataTableStyles = "margin-top: " + ((this.sHeaderHeight - 1) * -1) + "px;";
    ...
    if (this.fixedCols > 0) {
        ...
        //alert(this.sParentWidth - this.sFHeaderWidth);
