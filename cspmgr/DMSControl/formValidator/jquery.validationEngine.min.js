...
settings=jQuery.extend({allrules:allRules,validationEventTriggers:"focusout",inlineValidation:true,returnIsValid:false,liveEvent:true,unbindEngine:true,containerOverflow:false,containerOverflowDOM:"",ajaxSubmit:false,scroll:true,promptPosition:"topRight",success:false,beforeSuccess:function(){},failure:function(){}},settings);$.validationEngine.settings=settings;$.validationEngine.ajaxValidArray=new Array();if(settings.inlineValidation==true){if(!settings.returnIsValid){allowReturnIsvalid=false;if(settings.liveEvent){$(this).find("[class*=validate][type!=checkbox]").on(settings.validationEventTriggers,function(caller){_inlinEvent(this);})
...
$(".formError").on("click",function(){$(this).fadeOut(150,function(){$(this).remove()})})};$.validationEngine={defaultSetting:function(caller){if($.validationEngineLanguage){allRules=$.validationEngineLanguage.allRules;}else{$.validationEngine.debug("Validation engine rules are not loaded check your external file");}
...
_ajax(caller,rules,i);break;case"length":_length(caller,rules,i);break;case"maxCheckbox":_maxCheckbox(caller,rules,i);groupname=$(caller).attr("name");caller=$("input[name='"+groupname+"']");break;case"minCheckbox":_minCheckbox(caller,rules,i);groupname=$(caller).attr("name");caller=$("input[name='"+groupname+"']");break;case"confirm":_confirm(caller,rules,i);break;case"funcCall":_funcCall(caller,rules,i);break;case"datetimeFormat":_datetimeFormat(caller,rules,i);if($.validationEngine.isError)
i=rules.length;break;case"requiredField":_requiredField(caller,rules,i);break;case"compareDatetime":_compareDatetime(caller,rules,i);break;default:;};};radioHack();if($.validationEngine.isError==true){var linkTofieldText="."+$.validationEngine.linkTofield(caller);if(linkTofieldText!="."){if(!$(linkTofieldText)[0]){$.validationEngine.buildPrompt(caller,promptText,"error")}else{$.validationEngine.buildPrompt(caller,promptText,"error")}}else{$.validationEngine.closePrompt(caller);$.validationEngine.buildPrompt(caller,promptText,"error")}}else{$.validationEngine.closePrompt(caller);}
...
function _customRegex(caller,rules,position){customRule=rules[position+1];pattern=$.validationEngine.settings.allrules[customRule].regex;if(!pattern.test($(caller).attr('value'))){$.validationEngine.isError=true;promptText+=$.validationEngine.settings.allrules[customRule].alertText+"<br />";}}
...
function _funcCall(caller,rules,position){customRule=rules[position+1];funce=$.validationEngine.settings.allrules[customRule].nname;var fn=window[funce];if(typeof(fn)==='function'){var fn_result=fn();if(!fn_result){$.validationEngine.isError=true;promptText+=$.validationEngine.settings.allrules[customRule].alertText+"<br />";}}}
function _ajax(caller,rules,position){customAjaxRule=rules[position+1];postfile=$.validationEngine.settings.allrules[customAjaxRule].file;fieldValue=$(caller).val().replace(/'/g,"\\'");ajaxCaller=caller;fieldId=$(caller).attr("id");ajaxValidate=true;ajaxisError=$.validationEngine.isError;var mydata="{'validateId':'"+fieldId+"','validateValue':'"+fieldValue+"','validateError':'"+customAjaxRule+"'";var getIDValue=$.validationEngine.settings.allrules[customAjaxRule].getIDValue;if($.validationEngine.settings.allrules[customAjaxRule].getIDValue){getIDValue=$.validationEngine.settings.allrules[customAjaxRule].getIDValue;var getIDArr=getIDValue.split(",");for(var i=0;i<getIDArr.length;i++){mydata=mydata+","+"'"+getIDArr[i]+"':'"+$(("#"+getIDArr[i])).val()+"'";}}
if($.validationEngine.settings.allrules[customAjaxRule].extraData){extraData=$.validationEngine.settings.allrules[customAjaxRule].extraData;mydata+=","+extraData+"}";}else{extraData="";mydata+="}";}
if(!ajaxisError){$.ajax({type:"POST",url:postfile,async:false,data:mydata,contentType:"application/json; charset=utf-8",dataType:"json",beforeSend:function(){if($.validationEngine.settings.allrules[customAjaxRule].alertTextLoad){if(!$("div."+fieldId+"formError")[0]){return $.validationEngine.buildPrompt(ajaxCaller,$.validationEngine.settings.allrules[customAjaxRule].alertTextLoad,"load");}else{$.validationEngine.closePrompt(ajaxCaller);$.validationEngine.buildPrompt(ajaxCaller,$.validationEngine.settings.allrules[customAjaxRule].alertTextLoad,"load")}}},error:function(data,transport){$.validationEngine.debug("error in the ajax: "+data.status+" "+transport)},success:function(data){data=eval("("+data.d+")");ajaxisError=data.jsonValidateReturn[2];customAjaxRule=data.jsonValidateReturn[1];ajaxCaller=$("#"+data.jsonValidateReturn[0])[0];fieldId=ajaxCaller;ajaxErrorLength=$.validationEngine.ajaxValidArray.length;existInarray=false;if(ajaxisError=="false"){_checkInArray(false)
...
$.validationEngine.ajaxValid=false;if(typeof $.validationEngine.settings.allrules[customAjaxRule].alertText=="function")
promptText+=$.validationEngine.settings.allrules[customAjaxRule].alertText()+"<br />";else
promptText+=$.validationEngine.settings.allrules[customAjaxRule].alertText+"<br />";$.validationEngine.updatePromptText(ajaxCaller,promptText,"",true);}else{_checkInArray(true);$.validationEngine.ajaxValid=true;if(!customAjaxRule){$.validationEngine.debug("wrong ajax response, are you on a server or in xampp? if not delete de ajax[ajaxUser] validating rule from your form ")}
...
function _datetimeFormat(caller,rules,position){var checkType=parseInt(rules[position+1],10);var validateValue=$(caller).attr('value');if(DoCheckDatetime(checkType,validateValue)==false){$.validationEngine.isError=true;promptText+=$.validationEngine.settings.allrules["datetimeFormat"].alertText+"<br />";}}
function DoCheckDatetime(checkType,validateValue){var pattern=/\d{4}\/\d{2}\/\d{2}/;var checkResult=true;var my_date;var my_time;if(validateValue!=""){switch(checkType){case 0:pattern=/^\d{4}\/\d{2}\/\d{2}$/;if(validateValue.match(pattern)!=null){my_date=validateValue;checkResult=checkDate(my_date);}
else{checkResult=false;}
break;case 1:pattern=/^\d{4}\/\d{2}\/\d{2} \d{2}:\d{2}$/;if(validateValue.match(pattern)!=null){my_date=validateValue.split(" ")[0];my_time=validateValue.split(" ")[1];checkResult=checkDate(my_date);if(checkResult)
checkResult=checkTime(checkType,my_time);}
else{checkResult=false;}
break;case 2:pattern=/^\d{4}\/\d{2}\/\d{2} \d{2}:\d{2}:\d{2}$/;if(validateValue.match(pattern)!=null){my_date=validateValue.split(" ")[0];my_time=validateValue.split(" ")[1];checkResult=checkDate(my_date);if(checkResult)
checkResult=checkTime(checkType,my_time);}
else{checkResult=false;}
break;default:checkResult=false;break;}}
return checkResult;}
function lunyear(y,l)
{if(l<4){y=1911+y;}
if(y%400==0||(y%4==0)&&(y%100!=0))
return true;else
return false;}
function checkDate(dateString){var m_year;var m_month;var m_day;var m_year_lun;var temfun=true;m_year=parseInt(dateString.substring(0,4),10);m_month=parseInt(dateString.substring(5,7),10);m_day=parseInt(dateString.substring(8,10),10);m_year_lun=lunyear(m_year,4);if(m_year_lun==true){if(m_month==2&&m_day>29&&temfun==true){temfun=false;}}
else{if(m_month==2&&m_day>28&&temfun==true){temfun=false;}}
if(m_month>12||m_month<1){if(temfun==true){temfun=false;}}
if(m_month==4||m_month==6||m_month==9||m_month==11){if(m_day>30&&temfun==true){temfun=false;}
else{temfun=true;}}
if(m_day>31||m_day<1){if(temfun==true){temfun=false;}}
return temfun;}
function checkTime(Type,timeString){var m_hour;var m_minute;var m_second;var m_split=timeString.split(":");var temfun=true;switch(Type){case 1:m_hour=parseInt(m_split[0],10);m_minute=parseInt(m_split[1],10);if(m_hour>23||m_hour<0)
temfun=false;if(temfun){if(m_minute>59||m_minute<0)
temfun=false;}
break;case 2:m_hour=parseInt(m_split[0],10);m_minute=parseInt(m_split[1],10);m_second=parseInt(m_split[2],10);if(m_hour>23||m_hour<0)
temfun=false;if(temfun){if(m_minute>59||m_minute<0)
temfun=false;}
if(temfun){if(m_second>59||m_second<0)
temfun=false;}
break;default:temfun=false;break;}
return temfun;}
function _requiredField(caller,rules,position){var m_requiredField=$("#"+rules[position+1]);if($(caller).attr("value")==''&&$(m_requiredField).attr("value")==''){$.validationEngine.closePrompt("#"+$(caller).attr("id"));$.validationEngine.closePrompt("#"+$(m_requiredField).attr("id"));}
if($(caller).attr("value")!=''&&$(m_requiredField).attr("value")==''){$.validationEngine.loadValidation($(m_requiredField));}
if($(caller).attr("value")==''&&$(m_requiredField).attr("value")!=''){$.validationEngine.isError=true;promptText+=$.validationEngine.settings.allrules["requiredField"][$(caller).attr("id")]+"<br />";}}
function _compareDatetime(caller,rules,position){var m_chekType=parseInt(rules[position+1],10);var m_compareField_Start=$("#"+rules[position+2]);var m_compareField_End=$("#"+rules[position+3]);if(m_compareField_Start.val()!=''&&m_compareField_End.val()!=''){if(DoCheckDatetime(m_chekType,m_compareField_Start.val())&&DoCheckDatetime(m_chekType,m_compareField_End.val())){var tmpStartDate=new Date(m_compareField_Start.val());var tmpEndDate=new Date(m_compareField_End.val());if(tmpEndDate<tmpStartDate){$.validationEngine.isError=true;promptText+=$.validationEngine.settings.allrules["compareDatetime"][$(caller).attr("id")]+"<br />";}
else{if($(caller).attr("id")==m_compareField_Start.attr("id")){$.validationEngine.closePrompt("#"+m_compareField_End.attr("id"));}
else if($(caller).attr("id")==m_compareField_End.attr("id")){$.validationEngine.closePrompt("#"+m_compareField_Start.attr("id"));}}}}}
...
if(promptText!=""){var divFormError=document.createElement('div');var formErrorContent=document.createElement('div');linkTofield=$.validationEngine.linkTofield(caller)
...
return true;}else{return false;}},ValidationMove:function(caller){if($('.'+caller.attr('id')+'formError').length!=0)
$.validationEngine.loadValidation(caller);}}})(jQuery);