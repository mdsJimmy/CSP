...
public partial class oListView_Button : System.Web.UI.UserControl
{

    static log4net.ILog logger = log4net.LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);

    List<Collumn_Button> ListViewCollumns = new List<Collumn_Button>(); //欄位清單
    ...
    private void init_ListView()
    {
        ...
        //產生ListView的Template
        _ListView.LayoutTemplate = new CreateTemplate_Button(myListItemType_Button.Header, ListViewCollumns, DataKeyNames, IsUseCheckBox, OnClickExecFunc, TRTitle, IsUseBorder);
        _ListView.ItemTemplate = new CreateTemplate_Button(myListItemType_Button.Item, ListViewCollumns, DataKeyNames, IsUseCheckBox, OnClickExecFunc, TRTitle, IsUseBorder);
        _ListView.AlternatingItemTemplate = new CreateTemplate_Button(myListItemType_Button.AlternatingItem, ListViewCollumns, DataKeyNames, IsUseCheckBox, OnClickExecFunc, TRTitle, IsUseBorder);
        _ListView.EmptyDataTemplate = new CreateTemplate_Button(myListItemType_Button.EmptyData, ListViewCollumns, DataKeyNames, IsUseCheckBox, OnClickExecFunc, TRTitle, IsUseBorder);
...
struct Collumn_Button
{
    ...
    public string[] TypeButton;             //預設文字 "Button" 
}
...
/// </summary>
public enum myListItemType_Button
{
    // 摘要:
    //     清單控制項的頁首。這不是資料繫結的。
    Header = 0,
...
public class CreateTemplate_Button : System.Web.UI.ITemplate
{
    //System.Web.UI.WebControls.ListItemType templateType;
    myListItemType_Button templateType;
    List<Collumn_Button> ListViewCollumns = new List<Collumn_Button>();
    bool IsUseCheckBox = true;
    string DataKeyNames = "";
    string OnClickExecFunc = "";
    string TRTitle = "";
    bool IsUseBorder = false;
    public CreateTemplate_Button(myListItemType_Button type, object o, string sDataKeyNames, bool bIsUseCheckBox, string sOnClickExecFunc, string sTRTitle, bool bIsUseBorder)
    {
        templateType = type;
        ListViewCollumns = (List<Collumn_Button>)o;
    ...
    public void Item_DataBinding(object sender, System.EventArgs e)
    {
        ...
        for (int i = 0; i < ListViewCollumns.Count; i++)
        {
            ...
            if (ListViewCollumns[i].TypeButton != null)
            {
                ...
                if (itemValue.Equals("''"))
                {
                    ((Literal)ph.FindControl("_funonclickButton" + i)).Text = " type='hidden' value='' onclick=\"" + ListViewCollumns[i].TypeButton[1].Substring(0, ListViewCollumns[i].TypeButton[1].Length - 1) + "'" + KeyValue.ToString() + "');\"";
                }
                else if (!itemValue.Equals("isZip") && ListViewCollumns[i].TypeButton[0] == "zip" && ListViewCollumns[i].TypeButton[1] == "DoZip()")  //20170706 Add by Ellie [簽核表單交易狀況查詢]，欄位ATTACH_TYPE != 'zip'時，不做連結
                {
                    ((Literal)ph.FindControl("_funonclickButton" + i)).Text = " type='text' disabled='true' style='background - color:transparent; border: 0px; text-align:center' value=" + MDS.Utility.NUtility.checkString(itemValue.ToString()) + " " + ListViewCollumns[i].TypeButton[1].Substring(0, ListViewCollumns[i].TypeButton[1].Length - 1) + "'" + KeyValue.ToString() + "');\"";
                }
                else
                {
                    ((Literal)ph.FindControl("_funonclickButton" + i)).Text = " type='button' value=" + ListViewCollumns[i].TypeButton[0] + " onclick =\"" + ListViewCollumns[i].TypeButton[1].Substring(0, ListViewCollumns[i].TypeButton[1].Length - 1) + "'" + KeyValue.ToString() + "');\"";
                }
            }
            else
            {
