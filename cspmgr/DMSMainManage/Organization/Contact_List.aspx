<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/MasterPage/ContentPage.master"  CodeFile="Contact_List.aspx.cs" Inherits="DMSMainManage_Organization_Contact_List" %>
...
<asp:Content ID="Content4" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
 ...
 <asp:Label ID="Label_Address" runat="server" Text="" style='text-align:right'></asp:Label>
    ...
    <br>
        ...
        <%--ListView Start--%>
        <uc1:olistview ID="ContactList" runat="server" />
        <%--ListView Start--%>
...
<script type="text/javascript">
    function DoEdt(chkPK) {
        ...
        if (chkPK != '')
            parent.location.replace("Contact_edt.aspx?ContactID=" + tmpArr[1] + "&TargerGroupID=<%:TargerGroupID%>&PageNo=<%:PageNo%>&StrSearch=<%:mySearch%>");
    }


    function DoAdd() {
        parent.location.replace("Contact_add.aspx?TargerGroupID=<%:TargerGroupID%>&PageNo=<%:PageNo%>&StrSearch=<%:mySearch%>");
    }
    ...
    function DoDel() {
        ...
        var ContactIDList = GetCheckBoxValue();
        if (ContactIDList == '') {
            parent.MasterCtrl.showErrMsg('<%=ParseWording("A0009")%>');
        }
        else {
            $.confirm({
                ...
                confirm: function () {
                    PageMethods.Delete_ContactID(ContactIDList, '<%=MDS.Utility.NUtility.checkNull(Session["Name"])%>', '<%=MIPLibrary.MIPUtil.checkSessionName(Session["Name"],Session["UserID"])%>', ProcessSuccess, ProcessError);
                },
                cancel: function () {
                    //alert('Canceled!')
                }
    ...
    function ProcessSuccess(receiveData, userContext, methodName) {
        ...
        if (methodName == 'Delete_ContactID') {
            if (receiveData.nRet == 0) {
                parent.MasterCtrl.showSucessMsg(receiveData.returnData);
                parent.location.replace("Organization_Main.aspx?type=Contact");
            }
            else {
    ...
    /*搜尋*/
    function DoSearch() {
        parent.frmMain.location.replace("Contact_List.aspx?TargerGroupID=<%:TargerGroupID%>&PageNo=<%:PageNo%>&StrSearch=" + $('#StrSearch').val());
    }
    ...
    function DoChangeTag() {
        ...
        parent.frmMain.location.replace("Organization_List.aspx?TargerGroupID=<%:TargerGroupID%>");
    }
    ...
    function DoChangeTreeNode(TreeNode) {
        if (TreeNode.indexOf("document") == -1) {
            parent.frmMain.location.replace("../DMSMainManage/Organization/Contact_List.aspx?TargerGroupID=" + TreeNode);
